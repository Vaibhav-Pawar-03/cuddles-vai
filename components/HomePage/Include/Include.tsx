        ...
        <section>
            ...
            <div className="h-max bg-[#4AA6B1] px-4 md:px-14 flex flex-col items-center justify-center">
                <header className="mx-auto max-w-6xl px-6 py-10 text-white flex flex-col gap-8 justify-center items-center">
                    ...
                    <motion.h3 className="text-center text-balance text-2xl relative font-medium -mt-4" initial={{ y: -100, opacity: 0 }}  whileInView={{ y: 0, opacity: 1 }}  transition={{ duration: 0.6, delay: 0.6 }} viewport={{ once: false }}
                        >
                        We thoughtfully design every part of a child’s early years.
                    </motion.h3>
                </header>
                <div className='flex flex-col gap-14 justify-center items-center max-w-[600px]'>
                    {
                        content.map((item, ind) => {
                            return (
                                <div key={ind} className='grid md:grid-cols-2 grid-cols-1 md:grid-rows-1 grid-rows-2 gap-14'>
                                    <motion.div className='flex gap-4 md:items-left md:justify-center relative pl-10 md:pl-6' initial={{ x: -80, opacity: 0 }} whileInView={{ x: 0, opacity: 1 }} transition={{ duration: 0.7, delay: 0.1 * ind }} viewport={{ once: false }}>
                                        <img src={item.icon} alt={item.include} className='h-6' />
                                        <p className='text-white text-[14px] text-center'>{item.include}</p>
                                    </motion.div>
                                    <motion.div className='flex gap-4 md:items-center md:justify-center relative pl-10 md:pl-6' initial={{ x: 80, opacity: 0 }} whileInView={{ x: 0, opacity: 1 }} transition={{ duration: 0.7, delay: 0.1 * ind }} viewport={{ once: false }}>
